...
import { getCoverImageThumb } from '@/shared/lib/cover-image'
...
export function ArticleCard({ article, variant = 'default', className }: ArticleCardProps) {
  ...
  const imgSrc = article.coverImage || getCoverImageThumb(article.slug, article.category.slug)
    ...
    return (
      <Link href={`/articles/${article.slug}`} className={cn('group flex gap-4 items-start', className)}>
        <div className="relative w-24 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-zinc-100 dark:bg-zinc-800">
          <Image src={imgSrc} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
        </div>
        <div className="flex-1 min-w-0">
          <span className="text-xs font-semibold text-red-600 uppercase tracking-wide">
            {article.category.name}
          </span>
          <h3 className="mt-1 text-sm font-semibold text-zinc-900 dark:text-zinc-100 leading-snug line-clamp-2 group-hover:text-red-600 transition-colors">
            {article.title}
          </h3>
          <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-500">{publishedDate}</p>
        </div>
      </Link>
    )
  }
  ...
  if (variant === 'featured') {
    return (
      <Link href={`/articles/${article.slug}`} className={cn('group relative block rounded-xl overflow-hidden', className)}>
        <div className="relative h-[480px] bg-zinc-100 dark:bg-zinc-800">
          <Image src={imgSrc} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
