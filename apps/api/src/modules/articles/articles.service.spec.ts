...
describe('ArticlesService', () => {
  ...
  describe('update', () => {
    ...
    it('deve lançar NotFoundException quando artigo não existe', async () => {
      ...
      await expect(service.update('inexistente', {}, 'user-1', Role.ADMIN)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('unpublish', () => {
    ...
    it('deve lançar ForbiddenException para REDATOR', async () => {
      ...
      await expect(service.unpublish('article-1', 'user-1', Role.REDATOR)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('deve lançar NotFoundException quando artigo não existe', async () => {
      ...
      await expect(service.unpublish('inexistente', 'user-1', Role.ADMIN)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('archive', () => {
    ...
    it('deve lançar ForbiddenException quando REDATOR tenta arquivar artigo de outro', async () => {
      ...
      await expect(service.archive('article-1', 'user-1', Role.REDATOR)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('deve lançar NotFoundException quando artigo não existe', async () => {
      ...
      await expect(service.archive('inexistente', 'user-1', Role.ADMIN)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('findTrending', () => {
    it('deve retornar artigos PUBLISHED priorizando os últimos 7 dias', async () => {
      // Primeira chamada (7 dias): retorna o limite cheio -> não faz segunda chamada
      mockPrismaService.article.findMany.mockResolvedValue(
        Array.from({ length: 10 }, () => mockArticleBase),
      );
    ...
    it('deve completar com artigos all-time quando não há suficientes nos últimos 7 dias', async () => {
      // Primeira chamada (7 dias): retorna menos que o limite
      mockPrismaService.article.findMany
        .mockResolvedValueOnce([mockArticleBase])
        // Segunda chamada (all-time fallback)
        .mockResolvedValueOnce([{ ...mockArticleBase, id: 'article-2' }]);
